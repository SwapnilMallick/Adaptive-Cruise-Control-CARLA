...
class Predictor:
    def __init__(self, model_file: Path):
        ...
        self.model_file = torch.hub.load(
    ...
    def predict(self, obs) -> float:
        ...
        def test_simple(args):
            ...
            with torch.no_grad():
                ...
                input_image = Image.fromarray(obs)
                ...
                if args.pred_metric_depth:
                    metric_depth = 1.65 * depth.cpu().numpy()
                return metric_depth
        ...
        depth = test_simple(args)
        # print(depth.shape)

        # obs = cv2.resize(obs, (192, 640))

        results = self.model_file(obs)

        # print(len(results.xyxy[0]))
        if len(results.xyxy[0]) > 0:
            # width = results.xyxy[0][0][2] - results.xyxy[0][0][0]
            # height = results.xyxy[0][0][3] - results.xyxy[0][0][1]

            # pred_dist = (2 * 3.14 * 180) / (width + height * 360) * 1000 + 3
            # pred_dist = pred_dist * 0.254 - 6

            centroid_height = int(
                (results.xyxy[0][0][3] + results.xyxy[0][0][1]) / 2)
            centroid_width = int(
                (results.xyxy[0][0][2] + results.xyxy[0][0][0]) / 2)
            pred_dist = depth[0][0][centroid_height][centroid_width]

            #pred_dist = depth[0][0][int(results.xyxy[0][0][0]):int(results.xyxy[0][0][2]), int(results.xyxy[0][0][1]):int(results.xyxy[0][0][3])].mean()
